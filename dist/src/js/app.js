...
const dealerJSON = require('../../dealers.json');
...
const useful = {
  ...
  textCheckAdd(str, strToCheck) {
    // var expression1 = /(http|\/\/)/i;
    const expression1 = new RegExp(strToCheck, 'i');
    if (!str.match(expression1)) {
      return str = `${strToCheck}://${str}`;
    }
    return str;
  },
  textCheckRemove(str, strToCheck) {
    // var expression1 = /(http|\/\/)/i;
    const expression1 = new RegExp(strToCheck, 'i');
  ...
  time() {
    ...
    const date = new Date();
    const hrs = this.padZero(date.getHours());
...
const actions = {
  ...
  makeCard(info) {
    ...
    const sectionBottomLis = Object.keys(parsedHours).map(day => actions.makeElement({
      element: 'li',
    ...
    const footerLis = certifications.map(cert => actions.makeElement({
      element: 'li',
  ...
  async showCards(event) {
    ...
    if (clickedBoxes.length !== 0) {
      ...
      let dealerData;
      try {
        dealerData = await data.get();
      } catch (err) {
        console.error(`Received AJAX error, defaulting to backup.\n${err}`);
        dealerData = dealerJSON;
      }
      ...
      const dealerList = dealers || dealerJSON;
